...
public class TasmotaNodeController {
    ...
    private void configureSwitch(String nodeName, SwitchConfiguration config, DeviceName nodeDeviceName) {
        EntityId entityId = combine(
                config.getDevice(),
                new DeviceName(null, null, config.getEntity()),
                nodeDeviceName)
                        .toEntityId("Power");

        Device device = deviceManager.device(entityId.getDevice());

        new TasmotaSwitchDevice(nodeName, client, config.getRelay(), device.entity(new SwitchDefinition(entityId.getEntity())));
    }
