...
public class TasmotaSensorController {
    ...
    public void configure(TasmotaConfiguration config) {
        for (SensorConfiguration sensorConfiguration : config.getSensors()) {
            SensorTypeConfiguration sensorType = config
                    .getSensorTypes()
                    .getOrDefault(sensorConfiguration.getType(), DEFAULT_SENSOR_TYPE);
            configureSensor(sensorConfiguration, sensorType);
        }
    }

    private void configureSensor(SensorConfiguration sensorConfiguration, SensorTypeConfiguration sensorType) {
        DeviceId deviceId = sensorConfiguration.getDevice().toDeviceId();
        Device device = deviceManager.device(deviceId);

        TasmotaSensorDevice tasmotaSensor = new TasmotaSensorDevice(device, sensorConfiguration, sensorType);
