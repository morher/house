 ...
 *
 * <p>Items are internally stored in a list. The list can be retrieved through {@link #items()} or
 * just the last item through {@link #lastItem()}. Already received items can be cleared with {@link
 * #reset()}.
 *
 * <p>In cases where a test need to send an item and that message should not be recorded by the
 * TestSubscriber the item can be skipped through {@link #addSkip(Matcher)} or {@link
 * #addSkipEqual(Object)}.
 *
...
public class TestSubscriber<T> implements Closeable {
  ...
  /**
   * The constructor takes a {@link Subscribable}. It will subscribe to it and store the received
   * items in an internal list.
   *
   * @param subscribable
   */
  ...
  /**
   * Adds to the skip queue. Skipped items are not stored and will not be reported back by {@link
   * #items}, {@link #lastItem()} or {@link #size()}. The skipped items are checked against the
   * given {@link Matcher} to make sure the skipped message is the expected one.
   *
  ...
  /**
   * Convenience method for {@link #addSkip(Matcher)}, where the default {@link Matcher} {@code
   * is(equalTo(item))} is applied.
   *
   * @param item
   */
