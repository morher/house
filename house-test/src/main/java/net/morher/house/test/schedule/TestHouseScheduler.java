...
/**
 * An implementation of {@link HouseScheduler} that helps testing scheduled components. The
 * implementation simulates time and schedules tasks to be run in this time space.
 *
 * <p>The TestHouseScheduler lets the test code control the simulation of time progression through
 * {@link #skipTo(Instant)}, {@link #skipAhead(Duration)}, {@link TestHouseScheduler#skipAhead(long,
 * TemporalUnit)}, {@link #skipAhead(long, TimeUnit)} and {@link
 * #skipAheadToLastRegisteredTaskAndRunAllWaiting()}. Whenever time is skipped forward all tasks
 * that are scheduled to run in this time frame are run. If any task schedules new tasks to be run
 * within this time frame, they are run as well. All tasks are run in the chronological order. Tasks
 * that are scheduled to be run at the new current time are not run by the skip-methods, except
 * {@link #skipAheadToLastRegisteredTaskAndRunAllWaiting()}. To run tasks scheduled for "now", call
 * {@link #runWaitingTasks()}.
 *
 * <p>The current time can be retrieved by calling {@link #now()}. Notice that the code being tested
 * should also call {@link HouseScheduler#now()} to get the current time, and not {@link
 * Instant#now()}. Failing to do so will give unexpected results as two different time spaces will
 * be used.
 *
 * <p>{@link HouseScheduler#get()} should return this implementation when it is found on the
 * classpath. {@link TestHouseScheduler#get()} is provided as a convenience method for verifying and
 * casting.
 *
...
public class TestHouseScheduler implements HouseScheduler {
  ...
  /**
   * Get the global instance of {@link HouseScheduler} and verify the implementation is
   * TestHouseScheduler.
   *
   ...
   *
   * @param now The initial time for the scheduler.
   */
  ...
  /**
   * Skip ahead a given amount of time and run scheduled tasks due before the new now. It is only
   * possible to skip forwards in time.
   *
   ...
   * possible to skip forwards in time.
   *
   * @param duration The amount of time to skip
   * @return Itself
   */
  ...
  /**
   * Skip ahead to a given moment in time and run scheduled tasks due before the new now. It is only
   * possible to skip forwards in time.
   *
   * @param skipTo The moment to skip to
   * @return Itself
   */
  ...
  public void runWaitingTasks() {
    while (isNextTaskReady()) {
      ...
      task.runAndConsiderRescheduling().ifPresent(this::schedule);
    }
  }

  /**
   * Skip ahead to the time the last scheduled task is due to run and run all tasks in between and
   * at this time. Keep in mind that this includes new tasks scheduled by tasks being run. New tasks
   * that are scheduled to be run after the time skipped ahead to will not be run. Notice that the
   * skip-ahead is calculated before running the tasks, and new scheduling will not affect the
   * skip-ahead.
   */
  ...
  public ScheduledFuture<?> scheduleAtFixedRate(
    ...
    return schedule(
        new Task<>(
            command,
            now.plusNanos(unit.toNanos(initialDelay)),
  ...
  public void shutdown() {
    throw new UnsupportedOperationException(
        "scheduleWithFixedDelay not implemented in TestHouseScheduler");
  }
  ...
  public boolean awaitTermination(long timeout, TimeUnit unit) throws InterruptedException {
    throw new UnsupportedOperationException(
        "awaitTermination not implemented in TestHouseScheduler");
  }
  ...
  private boolean isNextTaskReady() {
    return nextTask != null && !nextTask.getRunAt().isAfter(now);
  }
  ...
  private class Task<T> implements ScheduledFuture<T> {
    ...
    private Task(
        ...
        boolean rescheduleRelativeToCompletion,
        T result) {
    ...
    public Task(
      ...
      this(
          ...
          runAt,
          rescheduleDelay,
          rescheduleRelativeToCompletion,
          null);
    }
