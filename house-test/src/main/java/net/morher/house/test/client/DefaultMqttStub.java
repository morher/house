...
public class DefaultMqttStub implements MqttStub {
  ...
  public <T> void simulateReceivedMessage(
      String topic, T payload, PayloadFormat<? super T> format) {
      ...
      for (Message message : retainedMessages.values()) {
        if (subscription.isMatch(message.getTopic())) {
          subscription
              .getListener()
              .onMessage(
                  message.getTopic(), message.getPayload(), message.getQos(), message.isRetain());
        }
      }
    }
    return subscription;
