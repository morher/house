...
public class CoverEntity extends CommandableEntity<CoverState, CoverOptions, CoverCommand> {
  private static final PayloadFormat<CoverState> STATE_FORMAT =
      EnumMessage.lowercase(CoverState.class);
  private static final PayloadFormat<CoverCommand> COMMAND_FORMAT =
      EnumMessage.lowercase(CoverCommand.class);
  ...
  public CoverEntity(HouseMqttClient client, EntityId entityId, EntityListener entityListener) {
    ...
    position =
        new StateCommandPair(
            client.topic(namespace.entityPositionTopic(entityId), NumberMessage.integer(), true),
            client.topic(
                namespace.entityPositionCommandTopic(entityId), NumberMessage.integer(), false));
    tilt =
        new StateCommandPair(
            client.topic(namespace.entityTiltTopic(entityId), NumberMessage.integer(), true),
            client.topic(
                namespace.entityTiltCommandTopic(entityId), NumberMessage.integer(), false));
  }
