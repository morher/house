...
public class HouseMqttClientImpl implements HouseMqttClient {
  ...
  public HouseMqttClientImpl(
      MqttOptions options, MqttNamespace namespace, MqttClientFactory clientFactory) {
    this.namespace = namespace;
    this.availabilityPolicy =
        new MqttAvailabilityPolicy(namespace.clientAvailabilityTopic(options.getClientId()));
  ...
  public void publishMessages() {
    ...
    try {
      while (!outgoingQueue.isEmpty()) {
        ...
        mqttConnection
            .client
            .publish(message.getTopic(), message.getPayload(), message.getQos(), message.isRetain())
            .waitForCompletion();
        outgoingQueue.poll();
      }
