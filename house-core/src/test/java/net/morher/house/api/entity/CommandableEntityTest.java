...
public class CommandableEntityTest {
  ...
  public void testSendCommand() {
    ...
    verify(client, times(1)).publish(topic.capture(), payload.capture(), retain.capture());
  ...
  public void testSubscribe() {
    ...
    ArgumentCaptor<MqttMessageListener> listenerCaptor =
        ArgumentCaptor.forClass(MqttMessageListener.class);
    verify(client, times(1))
        .subscribe(eq("house/room/device/entity/command"), listenerCaptor.capture());
  ...
  public static class TestCommandableEntity
      extends CommandableEntity<String, EntityOptions, String> {

    public TestCommandableEntity(
        HouseMqttClient client, EntityId entityId, EntityListener entityListener) {
